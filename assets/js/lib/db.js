...
function openDB(){
  return new Promise((resolve, reject)=>{
    ...
    req.onupgradeneeded=()=>{
      const db=req.result;
      if(!db.objectStoreNames.contains("rulesets")) db.createObjectStore("rulesets",{keyPath:"meta.id"});
...
export async function put(store, obj){ return tx(store,"readwrite", s=>s.put(obj)); }
export async function del(store, key){ return tx(store,"readwrite", s=>s.delete(key)); }
